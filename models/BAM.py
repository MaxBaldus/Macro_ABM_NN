...
class BAM:
    ...
    def simulation(self, theta, mc):
        ...
        """
        unemp_rate = np.zeros((self.T)) # unemployment rate
        unemp_growth_rate = np.zeros((self.T)) # unemployment growth rate 
        
        P_lvl = np.zeros((self.T)) # price level
        inflation = np.zeros((self.T)) # price inflation

        wage_lvl = np.zeros((self.T)) # wage level
        real_wage_lvl = np.zeros((self.T)) # real wage
        wage_inflation = np.zeros((self.T)) # wage inflation (not in %)
        avg_prod = np.zeros((self.T)) # average productivity of labor 
        product_to_real_wage = np.zeros((self.T)) # productivity/real wage ratio
        
        production = np.zeros((self.T)) # nominal gdp (quantities produced)
        output_growth_rate = np.zeros((self.T)) # output growth rate 
    
        aver_savings = np.zeros((self.T)) # average HH income 
        vac_rate = np.zeros((self.T)) # vacancy rate approximated by number of job openings and the labour force at the beginning of a period 
        bankrupt_firms_total = np.zeros((self.T)) # number of bankrupt firms in current round
        ...
        for t in range(self.T):
            ...
            for i in range(self.Nf):
                ...
                # otherwise: each firm decides output and price depending on last round s.t. it can actually determine its labor demand for this round
                    ...
                    prev_avg_p = P_lvl[t-1] # extract (current) average previous price 
            ...
            """
            P_lvl[t] = np.mean(p) # average price level 
            inflation[t] = ( P_lvl[t] - P_lvl[t-1] ) / P_lvl[t-1] if t!=0 else 0  

            """
            Output
            """
            production[t] = np.sum(Qp) # nominal GDP = sum of quantity produced by each firm
            output_growth_rate[t] = ( production[t] - production[t-1] ) / production[t-1] if t!=0 else 0

            """
            unemployment rate
            """
            unemp_rate[t] = 1 - (np.sum(L)/self.Nh)
            unemp_growth_rate[t] =  (unemp_rate[t] - unemp_rate[t-1] ) / unemp_rate[t-1] if t!=0 else 0

            """
            Wages
            """
            wage_lvl[t] = np.sum(W)/np.sum(L) # nominal wage level: sum of wage paid * number of employees relative to number of employees
            wage_inflation[t] = (wage_lvl[t]- wage_lvl[t-1]) / np.sum(wage_lvl[t-1]) if t!=0 else 0
            real_wage_lvl[t] = (1-inflation[t])*wage_lvl[t] if t!= 0 else 0 # (1-inflation rate)* wage = real income

            """
            Productivity / real wage ratio
            """
            avg_prod[t] = np.sum(alpha*L)/np.sum(L) # average productivity (only considering workers)
            product_to_real_wage[t] = avg_prod[t] / real_wage_lvl[t] if t!=0 else 0 # productivity relative to real wage lvl 

            """
            Vacancies
            """
            vac_rate[t] = np.round(np.sum(vac) / np.sum(L), decimals = 4) # vacancie rate is number of job openings (i.e. vacancies) relative to the labor force (labor employed currently)

            """
            Saving and consumption
            """
            aver_savings[t] = np.mean(S) # averge HH savings
            ...
            bankrupt_firms_total[t] = bankrupt_firms # save amount of bankrupt firms this round 
            ...
            add_row = [t, sum(Ld) / self.Nf, L_report, unemp_rate[t] ,sum(C)/np.mean(p), sum(NW), bankrupt_firms, sum(P), len(f_cr) ,min(p), 
                        max(p), sum(Qp), sum(S), sum(Qd), sum(Qr), sum(Qs), sum(C), wage_lvl[t], sum(E), sum(Cr)]
        ...
        if self.plots:
            ...
            # Philips curve
            plt.clf()
            plt.scatter(unemp_rate[:], wage_inflation[:])
            ...
            # Okuns law
            plt.clf()
            plt.scatter(unemp_growth_rate[:], output_growth_rate[:])
            ...
            # Beveridge
            plt.clf()
            plt.scatter(unemp_rate[:], vac_rate[:])
            plt.xlabel("Unemployment rate")
            ...
            # Log output
            plt.clf()
            plt.plot(np.log(production[self.T -499:]))
            plt.xlabel("Time")
            plt.ylabel("Log output")
            ...
            # unemployment 
            plt.clf()
            plt.plot(unemp_rate[self.T -499:])
            plt.xlabel("Time")
            plt.ylabel("unemployment rate")
            ...
            # productivity/real wage
            plt.clf()
            plt.plot(product_to_real_wage[self.T -499:])
            plt.xlabel("Time")
            plt.ylabel("Productivity-real wage")
            ...
            # Philips curve
            plt.clf()
            plt.scatter(unemp_rate[self.T -499:], wage_inflation[self.T -499:])
            plt.xlabel("Unemployment Rate")
            plt.ylabel("Wage Inflation")
            ...
            # Okuns law
            plt.clf()
            plt.scatter(unemp_growth_rate[self.T -499:], output_growth_rate[self.T -499:])
            plt.xlabel("Unemployment growth rate")
            plt.ylabel("Output growth rate")
            ...
            # Beveridge
            plt.clf()
            plt.scatter(unemp_rate[self.T -499:], vac_rate[self.T -499:])
            plt.xlabel("Unemployment rate")
            plt.ylabel("Vacancy rate")
            ...
            # bankrutpcy
            plt.clf()
            plt.plot(bankrupt_firms_total[self.T -499:])
            plt.xlabel("Time")
            plt.ylabel("bankruptcies")
            ...
            # mean HH Income
            plt.clf()
            plt.plot(aver_savings[self.T -499:])
            plt.xlabel("Time")
            plt.ylabel("average HH Income")
