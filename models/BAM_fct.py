...
def BAM_simulation(T, MC, plots, csv,
                Nh, Nf, Nb,
                theta):
    ...
    """        
    Z = 2 # Number of trials in the goods market (number of firms HH's randomly chooses to buy goods from)
    M = 4 # Number of trials in the labor market (number of firms one HH applies to)
    H = 2 # Number of trials in the credit market (Number of banks a firm selects randomly to search for credit)
    
    beta = 4 # MPC parameter that determines shape of decline of consumption propensity when consumption increases 
    Theta = 8 # Duration of individual contract
    r_bar = 0.01 # Base interest rate set by central bank (exogenous in this model)
    delta = 0.2 # Dividend payments parameter (fraction of net profits firm have to pay to the shareholders)
    capital_requirement_coef = 0.1 # capital requirement coefficients uniform across banks 
    ...
    """
    unemp_rate = np.zeros((T,MC)) # unemployment rate
    unemp_growth_rate = np.zeros((T,MC)) # unemployment growth rate 
    
    P_lvl = np.zeros((T,MC)) # price level
    inflation = np.zeros((T,MC)) # price inflation

    wage_lvl = np.zeros((T,MC)) # wage level
    real_wage_lvl = np.zeros((T,MC)) # real wage
    wage_inflation = np.zeros((T,MC)) # wage inflation (not in %)
    avg_prod = np.zeros((T,MC)) # average productivity of labor 
    product_to_real_wage = np.zeros((T,MC)) # productivity/real wage ratio
    
    production = np.zeros((T,MC)) # nominal gdp (quantities produced)
    output_growth_rate = np.zeros((T,MC)) # output growth rate 

    aver_savings = np.zeros((T,MC)) # average HH income 
    vac_rate = np.zeros((T,MC)) # vacancy rate approximated by number of job openings and the labour force at the beginning of a period 
    bankrupt_firms_total = np.zeros((T,MC)) # number of bankrupt firms in current round
    ...
    for mc in range(MC):
        ...
        # HOUSEHOLDS
        # Y = np.zeros(Nh) # income of HH
        S = np.zeros(Nh) # savings
        C = np.zeros(Nh) # (actual) consumption
        C_d = np.zeros(Nh) # desired / demanded consumption
        MPC = np.zeros(Nh) # marginal propensity to consume (declines with wealth)

        w = np.zeros(Nh) # wage of each HH
        w_last_round = np.zeros(Nh) # wage of round before 
        unemp_benefit = np.zeros(Nh) # uneployment benefits in case HH has no employment
        div = np.zeros(Nh) # dividend payment to HH from profit of firm (new each round)
        w_flag = np.zeros(Nh) # flag in case HH received wage in current t
        div_flag = np.zeros(Nh) # flag in case HH received dividend payment in current t

        is_employed = np.zeros(Nh, bool) # True if HH has job: e.g. array([False, False, False, False, False])
        d_employed = np.zeros(Nh) # counts the days a HH is employed
        d_unemployed = np.zeros(Nh) # counts the days a HH is unemployed
        #firm_id = np.zeros(Nh) # firm_id where HH is employed
        prev_firm_id = np.zeros(Nh, int) # firm id HH was employed last
        firms_applied =  [[] for _ in range(Nh)] # list of firm ids HH applied to 
        job_offered_from_firm = [[] for _ in range(Nh)] # list of an offered job from firm i for each HH

        #firm_went_bankrupt = np.zeros(Nh) # flag (value of 1) in case the firm HH was employed just went bankrupt 
        expired = np.zeros(Nh, bool) # flag in case contract of a HH expired, it is set to True 
        goods_visited = np.zeros(Nh)# one list for each HH in which firm id's are stored that sold product to HH in round before
        
        # FIRMS
        Qd = np.zeros(Nf) # Desired qty production : Y^D_it = D_ie^e
        Qp = np.zeros(Nf) # Actual Qty produced: Y_it
        Qs = np.zeros(Nf) # Qty sold
        Qr = np.zeros(Nf) # Qty Remaining
        rho = np.zeros(Nf) # intiaize Qty update parameter
        
        eta = np.zeros(Nf) # Price update random number to be sampled later
        p = np.zeros(Nf) # price
        
        Ld = np.zeros(Nf) # Desired Labor to be employed
        Lhat = np.zeros(Nf) # Labor whose contracts are expiring
        L = np.zeros(Nf)  # actual Labor Employed
        Lo = np.zeros(Nf) # actual workforce (L of last round - amount of just expired contracts)
        vac = np.zeros(Nf) # Vacancy
        w_emp = [[] for _ in range(Nf)] # Ids of workers/household employed - each firm has a list with worker ids
        W = np.zeros(Nf) # aggregated Wage bills (wage payments to each Worker * employed workers)
        w_min_slice = np.repeat(np.arange(1,T,4), 4)  # array with values from 3 up to T-3 appearing 4 times in a row for slicing the minimum wage s.t. min. wage is revised every 4 periods
        time_stamp_wage_adjustment = np.arange(3,T,4)
        Wp = np.zeros(Nf) # Wage payments of firm this round
        job_applicants = [[] for _ in range(Nf)] # Ids of Job applicants (HH that applied for job) -> Liste für each firm => array with [] entries for each i
        
        is_hiring = np.zeros(Nf, bool) # Flag to be activated when firm enters labor market to hire 

        P = np.zeros(Nf) # net Profits
        NW = np.ones(Nf)  # initial Net worth
        Rev = np.zeros(Nf) # Revenues of a firm (gross profits)
        RE = np.zeros(Nf) # Retained Earnings (net profits - divididens)
        divs = np.zeros(Nf) # Dividend payments to the households

        B = np.zeros(Nf) # amount of desired credit (total)
        lev = np.zeros(Nf) # invdivudal leverage (debt ratio)
        loan_paid = np.zeros(Nf)
        credit_appl = [[] for _ in range(Nf)] # list with bank ids each frim chooses randomly to apply for credit
        #loans_to_be_paid = np.zeros(Nf) # outstanding debt + interest rate firm has to pay back to bank 

        outstanding = [[] for _ in range(Nf)] # under 6) if firm cannot pay back entire loan(s), amount outstanding for each bank (not paid back) is saved here
        outstanding_flag = np.zeros(Nf) # under 6), set to 1 if firm cannot pay back entire amount of loans
        outstanding_to_bank = [[] for _ in range(Nf)] # save the bank ids each firm has outstanding amount to 
        outstanding_r = [[] for _ in range(Nf)] # interest rate charged by bank where firm has outstanding credit

        Qp_last = np.zeros(Nf) # variable to save quantity produced in round before 
        bankrupt_flag = np.zeros(Nf) # set entry to 1 in case firm went bankrupt 

        # BANKS
        E = np.zeros(Nb) # equity base 
        phi = np.zeros(Nb)
        tCr = np.zeros(Nb) # initial amount of Credit of all banks, filled with random numbers for t = 0
        Cr = np.zeros(Nb) # amount of credit supplied / "Credit vacancies"
        Cr_a = np.zeros(Nb) # actual amount of credit supplied (after..)
        Cr_rem = np.zeros(Nb) # credit remaining 
        # Cr_s = [[] for _ in range(Nb)] # amount of credit supplied 
        # r_s = [[] for _ in range(Nb)] # interest rate supplied 
        
        firms_applied_b = [[] for _ in range(Nb)] 
        # firms_loaned = [[] for _ in range(Nb)] 
        Cr_p = np.zeros(Nb)

        Bad_debt = np.zeros(Nb)
        ...
        """
        # HOUSEHOLDS
        S = np.around(np.random.uniform(low=1,high=1,size=Nh), decimals = 4) # initial (random) income of each HH  
        # S_initial = 10 => sum(C_d) 3179 ; S_intiial = 2 => sum(Qr) = 25
        MPC = np.around(np.random.uniform(low=0.6,high=0.9,size=Nh), decimals = 4) # initial random marginal propensity to consume of each HH 

        # FIRMS
        NW = np.around(np.random.uniform(low=5 ,high=5, size=Nf), decimals = 4) # initial net worth of the firms 
        ...
        p = np.around(np.random.uniform(low=1.4, high=1.4,size=Nf), decimals = 4) # initial prices
        ...
        Wp = np.around(np.random.uniform(low= 1,high=1,size=Nf), decimals = 4) # initial wages
        alpha = np.random.uniform(low=0.5, high=0.5, size = Nf) # Productivity alpha stays constant here, since no R&D in this version
        ...
        # BANKS
        E = np.random.uniform(low = 5, high = 5, size = Nb)
        ...
        for t in range(T):
            ...
            eta = np.random.uniform(low=0,high=H_eta, size = Nf) # sample value for the (possible) price update (growth rate) for the current firm 
            rho = np.random.uniform(low=0,high=H_rho, size = Nf) # sample growth rate of quantity of firm i
            for i in range(Nf):
            ...
            c_ids = np.arange(1, Nh + 1, 1, dtype=int) # array with consumer ids
            ...
            for i in range(Nf):
                ...
                for j in (HH_employed): # j takes on HH id employed within current firm i 
                    if d_employed[j-1] > Theta: # if contract expired (i.e. days employed is greater than 8) 
            ...
            if len(f_empl) > 0: # if there are firms that have open vacancies, search and match begins
                ...
                for j in c_ids:
                    ...
                    M_current = M # number of firms HH visits
                    ...
                    if len(f_empl_current) > M:
                        chosen_firms = list(np.random.choice(f_empl_current, M_current, replace = False)) # random chosen M firms the HH considers
            ...
            banks = [[] for _ in range(Nf)] # list of banks from where the firm got the loans (nested list for each firm)
            Bi = [[] for _ in range(Nf)] # Amount of credit taken by banks
            r_f = [[] for _ in range(Nf)] # rate of Interest on credit by banks for which match occured (only self.r bei ihm) 
            
            r = [[] for _ in range(Nb)] # list of interest rate bank determines for each firm 
            ...
            if len(f_cr) > 0:
                ...
                for b in range(Nb):
                    ...
                    Cr[b] = np.around(E[b] / capital_requirement_coef, decimals=4) # amount of credit that can be supplied is an multiple of the equity base: E
                    ...
                    for f in range(Nf):
                        current_r = r_bar*(1 + (phi[b] * np.sqrt(lev[f]))) # current interest rate 
                ...
                for f in f_cr:
                    ...
                    chosen_banks = list(np.random.choice(b_cred, H, replace = False)) # firm randomly chooses H banks
            ...
            c_list = [1+f for f in range(Nh)] # initialize list of consumers
            ...
            for c in c_list:
                ...
                MPC[c-1] = np.around(1/(1 + (np.tanh((S[c-1])/savg))**beta),decimals=4) if t > 0 else MPC[c-1] 
                ...
                if bankrupt_flag[prev_firm - 1] == 0 and prev_firm in f_produced and t > 0:
                    Z_current = Z - 1  # Z reduces by 1 due to preferential attachmend in case firm from last round did not went bankrupt 
                ...
                random_firms = list(np.random.choice(f_produced_current, Z_current, replace = False)) # HH randomly chooses Z firms
            ...
            for f in range(Nf):
                ...
                if len(banks_f) > 0:
                    ...
                    if Rev[f] >= loans_to_be_paid:
                        ...
                        divs[f] = np.around(P[f]*delta , decimals = 8) # dividends are profits * delta (share), if firm f has positive profits
                        NW[f] = np.around(NW[f] + (1-delta)*P[f], decimals = 8) # udpate firms NW
            ...
            for c in range(Nh):
                ...
                div[c] = div[c] + np.around(divs[f]/Nh ,decimals=4)
            ...
            for i in NW:
                if NW[i] < 0:
                    bankrupt_firms = bankrupt_firms + 1
            ...
            """add_row = [t, sum(Ld) / Nf, L_report, unemp_rate[t,mc] ,sum(C)/np.mean(p), sum(NW), bankrupt_firms, sum(P), len(f_cr) ,min(p), 
            ...
            if t in [T-499,T-599,T-699,T-799,T-899,T-999]:
        ...
        if plots:
            ...
            # Log output
            plt.clf()
            plt.plot(np.log(production[T -499:,mc]))
            plt.xlabel("Time")
            plt.ylabel("Log output")
            ...
            # productivity/real wage
            plt.clf()
            plt.plot(product_to_real_wage[T -499:,mc])
            plt.xlabel("Time")
            plt.ylabel("Productivity-real wage")
            ...
            # Philips curve
            plt.clf()
            plt.scatter(unemp_rate[T -499:,mc], wage_inflation[T -499:,mc])
            plt.xlabel("Unemployment Rate")
            plt.ylabel("Wage Inflation")
            ...
            # Okuns law
            plt.clf()
            plt.scatter(unemp_growth_rate[T -499:,mc], output_growth_rate[T -499:,mc])
            plt.xlabel("Unemployment growth rate")
            plt.ylabel("Output growth rate")
            ...
            # Beveridge
            plt.clf()
            plt.scatter(unemp_rate[T -499:,mc], vac_rate[T -499:,mc])
            plt.xlabel("Unemployment rate")
            plt.ylabel("Vacancy rate")
            ...
            # bankrutpcy
            plt.clf()
            plt.plot(bankrupt_firms_total[T -499:,mc])
            plt.xlabel("Time")
            plt.ylabel("bankruptcies")
            ...
            # mean HH Income
            plt.clf()
            plt.plot(aver_savings[T -499:,mc])
            plt.xlabel("Time")
            plt.ylabel("average HH Income")
