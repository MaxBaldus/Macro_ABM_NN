...
spec = [
    ('MC', int32),               # a simple scalar field
    ('T', int32), 
    ('Nh', int32),
    ('Nf', int32),
    ('Nb', int32), 
    ...
    ('r_bar', int32),
    ('delta', int32),    
    ('capital_requirement_coef', int32), 
    ('theta', float32[:]),
]
...
class BAM:
    ...
    def simulation(self):
        ...
        for mc in tqdm(range(self.MC)):
            ...
            @staticmethod
            def simulate_numba(H_eta, H_rho, H_phi, h_xi,
                               self):
                
                P_lvl = np.zeros(self.T)
                inflation = np.zeros(self.T)
                production = np.zeros(self.T)
                unemp_rate = np.zeros(self.T)
                ...
                # P_lvl, inflation, production, unemp_rate
                return production

            # save each model run (each mc replication)
            production[:,mc] = simulate_numba(self, H_eta, H_rho, H_phi, h_xi) 
